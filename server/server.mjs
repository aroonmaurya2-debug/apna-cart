...
app.post('/api/orders', async (request, response) => {
  ...
  const productTotal = items.reduce((sum, item) => sum + (Number(item.price) || 0) * (Number(item.quantity) || 1), 0); const { commission, sellerAmount } = calculateCommission(productTotal)
  const order = { id: Date.now(), createdAt: new Date().toISOString(), customer: { name: session.name, contact: session.contact, phone: phone.trim(), email: email?.trim() || '' }, address: address.trim(), paymentMethod: paymentMethod || 'Cash on Delivery', total: Number(total) || productTotal, productTotal, commission, sellerAmount, platformEarning: commission, earningStatus: 'Pending', items, status: 'Processing', location: 'Order received' }
...
app.patch('/api/orders/:id/status', async (request, response) => {
  ...
  const nextStatus = String(request.body?.status || '').trim(); const nextLocation = String(request.body?.location || '').trim(); const allowedStatuses = new Set(['Processing', 'Accepted', 'Shipped', 'Delivered'])
  ...
  try {
    if (ordersCollection) { const reference = ordersCollection.doc(request.params.id); const snapshot = await reference.get(); if (!snapshot.exists) return response.status(404).json({ message: 'Order not found.' }); const order = snapshot.data(); await reference.update({ status: nextStatus, location: nextLocation }); return response.json({ order: { id: snapshot.id, ...order, status: nextStatus, location: nextLocation } }) }
    const orders = await readOrders(); const order = orders.find((item) => String(item.id) === request.params.id); if (!order) return response.status(404).json({ message: 'Order not found.' }); order.status = nextStatus; order.location = nextLocation; await writeOrders(orders); return response.json({ order })
