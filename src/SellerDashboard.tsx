...
const inputStyle: React.CSSProperties = { width: '100%', padding: '11px 12px', border: '1px solid #d5e0da', borderRadius: 9, background: '#fff', fontSize: 14, boxSizing: 'border-box' }
...
export default function SellerDashboard() {
  ...
  return <div style={{ minHeight: '100vh', background: '#f4f7f5', color: '#173a2b', fontFamily: 'system-ui, sans-serif' }}>
    ...
    <div style={{ maxWidth: 1250, margin: '0 auto', display: 'flex', minHeight: 'calc(100vh - 64px)' }}>
      <aside style={{ width: 220, background: '#fff', padding: 14, borderRight: '1px solid #e1e9e4' }}>
        {nav.map(([id, label]) => <button key={id} onClick={() => setTab(id)} style={{ width: '100%', textAlign: 'left', border: 0, borderRadius: 9, padding: 12, marginBottom: 5, background: tab === id ? '#e3f6ed' : 'transparent', color: tab === id ? '#078a58' : '#465a51', fontWeight: 700 }}>{label}</button>)}
        <button onClick={() => { window.location.href = '/' }} style={{ width: '100%', textAlign: 'left', border: 0, background: 'transparent', padding: 12 }}>← Back to Shopping</button>
      </aside>
      <main style={{ flex: 1, padding: 24 }}>
        {message && <div style={{ ...cardStyle, marginBottom: 15 }}>{message}</div>}
        {tab === 'overview' && <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, margin: '20px 0' }}>
            {[['Sales', money(revenue)], ['Orders', String(orders.length)], ['Products', String(products.length)], ['Earnings', money(earnings)]].map(([a, b]) => <div key={a} style={cardStyle}><small>{a}</small><h2>{b}</h2></div>)}
          </div>
          <div style={cardStyle}><h3>Recent Orders</h3>{orders.length === 0 ? <p>No orders yet.</p> : orders.slice(0, 5).map(o => <div key={o.id} style={{ padding: '10px 0', borderBottom: '1px solid #eee' }}><b>#{String(o.id).slice(-10)}</b> — {o.customerName || 'Customer'} — {money(Number(o.total || 0))} — {o.status || 'Placed'}</div>)}</div>
        </>}
        {tab === 'products' && <>
          <h1>Products</h1><p>Add and manage products listed on Apna Cart.</p>
          <form onSubmit={addProduct} style={{ ...cardStyle, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            ...
            <input placeholder="Sizes: S,M,L,XL" value={form.sizes} onChange={e => setField('sizes', e.target.value)} style={inputStyle} />
            <input placeholder="Colors: Red,Blue" value={form.colors} onChange={e => setField('colors', e.target.value)} style={inputStyle} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>{products.map(p => <div key={p.id} style={{ ...cardStyle, padding: 0, overflow: 'hidden' }}><img src={p.image} alt={p.name} style={{ width: '100%', height: 180, objectFit: 'cover' }} /><div style={{ padding: 12 }}><b>{p.name}</b><p>{money(Number(p.price))} • Stock: {Number(p.stock || 0)}</p><small>{p.category} • {p.gender || 'Unisex'}</small><br /><button onClick={() => deleteProduct(p.id)} style={{ ...buttonStyle, background: '#fff', color: '#c33', border: '1px solid #ddd', marginTop: 10 }}>Delete</button></div></div>)}</div>
        </>}
