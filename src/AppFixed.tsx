...
const products: Product[] = [
  ...
  { id: 5, name: 'Everyday Backpack', category: 'Bags', price: 899, oldPrice: 1799, image: 'https://images.unsplash.com/photo-1553062407-98eeb64c6a62?auto=format&fit=crop&w=800&q=85', rating: 4.2, reviews: 45, discount: '50% OFF' },
  ...
  { id: 8, name: 'Matte Lipstick', category: 'Beauty', price: 299, oldPrice: 599, image: 'https://images.unsplash.com/photo-1586495777744-4413f21062fa?auto=format&fit=crop&w=800&q=85', rating: 4.4, reviews: 72, discount: '50% OFF' },
]
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {view === 'home' && <>
        ...
        <section className="category-strip">{categories.map(([label, icon]) => <button key={label} className={`category-item ${label === 'All Categories' ? category === 'All' ? 'active' : '' : ''}`} onClick={() => { const map: Record<string, string> = { Men: 'Fashion', Beauty: 'Beauty', Footwear: 'Footwear', Home: 'Home' }; setCategory(map[label] || 'All') }}><span className="category-icon">{icon}</span><span>{label}</span></button>)}</section>
        ...
        <section className="section-heading"><h2>Trending For You</h2><button onClick={() => { setCategory('All'); setQueryText('') }}>View All →</button></section>
        <section className="product-grid">{filtered.map((p, index) => <article key={p.id} className="product-card"><div className="product-image-wrap"><span className="product-tag">{index % 3 === 0 ? 'Best Seller' : index % 3 === 1 ? 'Top Rated' : 'Trending'}</span><button className="heart" onClick={() => setMessage('Wishlist feature coming soon.')} aria-label="Wishlist">♡</button><img src={p.image} alt={p.name} /></div><div className="product-info"><h3>{p.name}</h3><div className="price-row"><strong>{money(p.price)}</strong><del>{money(p.oldPrice)}</del><span className="discount">{p.discount}</span></div><div className="cod">{money(Math.round(p.price * 1.1))} with COD</div><div className="rating"><b>★ {p.rating}</b> <span>({p.reviews.toLocaleString('en-IN')})</span></div><button className="add-button" onClick={() => { addToCart(p.id); setMessage(`${p.name} cart me add ho gaya.`) }}>🛒 Add to Cart</button></div></article>)}</section>
      </>}
    ...
    <nav className="bottom-nav"><button className={view === 'home' ? 'active' : ''} onClick={goHome}><span>⌂</span>Home</button><button onClick={() => { setView('home'); setCategory('All') }}><span>▦</span>Categories</button><button className={view === 'cart' ? 'active' : ''} onClick={() => setView('cart')}><span>🛍️<i>{cartCount}</i></span>Cart</button><button onClick={() => setMessage('Wishlist feature coming soon.')}><span>♡</span>Wishlist</button><button className={view === 'orders' ? 'active' : ''} onClick={() => setView('orders')}><span>📦</span>My Orders</button></nav>
  </div>
}
