...
export default function AdminPanel() {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.some(f => f.size > 3 * 1024 * 1024)) { setError('Har image 3MB se chhoti honi chahiye.'); return }
    Promise.all(files.map(file => new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result || '')); reader.onerror = () => reject(new Error('Image read failed')); reader.readAsDataURL(file) }))).then(images => setForm((p: any) => ({ ...p, image: images[0] || '', images }))).catch(() => setError('Images select nahi ho payi.'))
  }
  ...
  const field = (label: string, key: string, type = 'text') => <label><span>{label}</span><input type={type} value={form[key] ?? ''} onChange={e => setForm({ ...form, [key]: e.target.value })} /></label>
