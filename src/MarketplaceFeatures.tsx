...
export default function MarketplaceFeatures() {
  ...
  useEffect(() => { if (open) { void loadProducts(); void loadOrders(); void loadReferral() } }, [open])
  ...
  async function loadOrders() { try { const data = await api('/orders'); setOrders(data.orders || []) } catch { setOrders([]) } }
  async function loadReferral() { try { const data = await api('/referrals/me'); setReferral(data.referral || null) } catch { setReferral(null) } }
  async function cancelOrder(id: string) { try { await api('/orders/'+id+'/cancel', { method:'POST' }); await loadOrders(); setMessage('Order cancel request submit ho gaya.') } catch(e) { setMessage(e instanceof Error ? e.message : 'Cancel nahi hua.') } }
  async function returnOrder(id: string) { const reason = window.prompt('Return reason likhiye:')?.trim(); if (!reason) return; try { await api('/orders/'+id+'/return', { method:'POST', body: JSON.stringify({ reason }) }); await loadOrders(); setMessage('Return request submit ho gaya.') } catch(e) { setMessage(e instanceof Error ? e.message : 'Return request nahi hua.') } }
  ...
  return <>
    ...
    {open && <div className="marketplace-overlay" onClick={() => setOpen(false)}><section className="marketplace-panel" onClick={e => e.stopPropagation()}>
      ...
      {section === 'help' && <div className="marketplace-content"><h4>📦 My Orders</h4><div className="seller-actions"><button onClick={()=>setOrderFilter('all')}>All</button><button onClick={()=>setOrderFilter('active')}>Active</button><button onClick={()=>setOrderFilter('cancelled')}>Cancelled</button></div>{orders.filter(o=>orderFilter==='all'||(orderFilter==='cancelled'?o.status==='cancelled':o.status!=='cancelled')).map(o=><div className="marketplace-order" key={String(o.id)}><b>#{o.id}</b><span>{o.status}</span><strong>₹{Number(o.total||0).toLocaleString('en-IN')}</strong>{['placed','pending','confirmed'].includes(o.status)&&<button onClick={()=>cancelOrder(String(o.id))}>Cancel</button>}{['delivered','completed'].includes(o.status)&&!o.returnStatus&&<button onClick={()=>returnOrder(String(o.id))}>Return</button>}{o.returnStatus&&<small>Return: {o.returnStatus}{o.refundStatus?` • Refund: ${o.refundStatus}`:''}</small>}</div>)}{orders.length===0&&<p>No orders yet.</p>}<hr/><h4>🎁 Referral & Rewards</h4>{referral?<><p>Your referral code: <b>{referral.code}</b></p><p>Successful invites: {referral.successfulInvites||0} · Balance: ₹{referral.balance||0}</p></>:<p>Login karke referral code dekhein.</p>}</div><div className="marketplace-content"><h4>🛡️ Safe Shopping</h4><p>Product details, seller name aur price check karke order karein. Order place hone ke baad My Orders me status track karein.</p><p>Abhi working payment option <b>Cash on Delivery</b> hai. UPI/Card gateway baad me securely connect kiya ja sakta hai.</p></div>}
