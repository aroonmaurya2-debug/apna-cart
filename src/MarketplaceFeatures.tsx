...
export default function MarketplaceFeatures({ onMessage }: Props) {
  ...
  const registerSeller = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/sellers', { method: 'POST', headers: { 'content-type': 'application/json', ...(token ? { authorization: `Bearer ${token}` } : {}) }, body: JSON.stringify({ sellerId: `seller-${Date.now()}`, name: sellerName.trim(), email: sellerEmail.trim() }) })
    ...
    } catch (error) { onMessage?.(error instanceof Error ? error.message : 'Seller registration failed.') }
    finally { setSaving(false) }
  }

  return <>
    <button className="marketplace-fab" onClick={() => setOpen(true)} aria-label="Marketplace options">☰</button>
    {open && <div className="marketplace-overlay" onClick={() => setOpen(false)}><section className="marketplace-panel" onClick={event => event.stopPropagation()}>
      <div className="marketplace-panel-head"><h3>Apna Cart Marketplace</h3><button onClick={() => setOpen(false)}>✕</button></div>
      <div className="marketplace-tabs"><button className={section === 'help' ? 'active' : ''} onClick={() => setSection('help')}>Help</button><button className={section === 'seller' ? 'active' : ''} onClick={() => setSection('seller')}>Sell</button><button className={section === 'wishlist' ? 'active' : ''} onClick={() => setSection('wishlist')}>Wishlist</button></div>
      {section === 'help' && <div className="marketplace-content"><h4>Shopping safely</h4><p>Product details check karein, delivery address sahi bharein aur order status My Orders se track karein.</p><p>Payment gateway connect hone tak Cash on Delivery primary option hai.</p></div>}
      {section === 'wishlist' && <div className="marketplace-content"><h4>Wishlist</h4><p>Wishlist area ready hai. Product-level wishlist ko catalog data integration ke saath connect kiya jayega.</p></div>}
