(function(){
  ...
  function openMall(){
    ...
    overlay.innerHTML='<section class="apna-mall-sheet" role="dialog" aria-modal="true" aria-label="Apna Cart Mall">'
      ...
      +'<button type="button" data-mall-cat="Saree"><b>👗</b><span>Fashion</span><small>Saree & new arrivals</small></button>'
      ...
      +'<button type="button" data-mall-cat="Kids"><b>🧸</b><span>Kids</span><small>Fun & comfy</small></button>'
      +'<button type="button" data-mall-cat="Footwear"><b>👟</b><span>Footwear</span><small>Daily comfort</small></button>'
      +'<button type="button" data-mall-cat="Home"><b>🏠</b><span>Home</span><small>Fresh home picks</small></button>'
      +'<button type="button" data-mall-cat="All Categories"><b>▦</b><span>All Categories</span><small>Explore everything</small></button>'
    ...
    overlay.addEventListener('click',function(e){
      ...
      if(strip){
        var buttons=Array.from(strip.querySelectorAll('button'));
        var target=buttons.find(function(x){return (x.textContent||'').toLowerCase().indexOf(cat.toLowerCase())!==-1});
        if(target){close();target.click();strip.scrollIntoView({behavior:'smooth',block:'start'});return}
      }
      close();
    });
  }
