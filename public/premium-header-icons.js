(function(){
  'use strict';
  function getPhoto(){try{return localStorage.getItem('apna-cart-profile-photo')||''}catch(e){return ''}}
  function polish(){
    ...
    if(p){
      p.classList.add('premium-profile-icon');
      p.setAttribute('aria-label','Profile');
      p.title='Profile';
      var photo=getPhoto();
      if(photo){
        p.style.setProperty('background-image','url("'+photo.replace(/"/g,'&quot;')+'")','important');
  ...
  s.textContent=`
    ...
    .header .premium-profile-icon{border-radius:50%!important;overflow:hidden!important;background-color:#e6f6ee!important;background-repeat:no-repeat!important}
    .header .premium-profile-icon::before{content:'👤'!important;font-size:20px!important}
    .header .premium-profile-icon[style*="background-image"]::before{content:''!important}
  ...
  function start(){polish();new MutationObserver(polish).observe(document.body,{childList:true,subtree:true});window.addEventListener('apna-cart-profile-photo-updated',polish);}
