(() => {
  ...
  const notify = (title, body, type='info') => window.apnaNotify?.add?.(title, body, type)
  const ensureStyles = () => {
  ...
  const orderId = text => String(text||'').match(/order\s*#?\s*(\d{5,})/i)?.[1] || ''
  ...
  const enhance = () => {
    ...
    candidates.slice(0,12).forEach(card=>{
      ...
      const id=orderId(text), current=statusIndex(text), box=document.createElement('div'); box.className='apna-order-progress'
      ...
      actions.addEventListener('click',async e=>{
        ...
        try {
          ...
          if(action==='cancel'){message.textContent='Cancellation requested successfully.';notify('Order cancelled',`Order #${id} cancellation request submitted.`,'order');box.querySelector('.apna-order-progress-title').textContent='❌ Order cancelled';button.parentElement.remove()}
          else if(action==='return'){message.textContent='Return request submitted. We will review it.';notify('Return requested',`Return request for order #${id} was submitted.`,'return');button.textContent='Return requested'}
          else {const amount=Number(data.order?.total||0).toLocaleString('en-IN');message.textContent=`Refund request submitted for ₹${amount}.`;notify('Refund requested',`Refund request for order #${id} was submitted.`,'refund');button.textContent='Refund requested'}
