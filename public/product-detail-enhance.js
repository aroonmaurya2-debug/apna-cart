(() => {
  ...
  const enhance=()=>{
    ...
    // Hide the original React action row (its buttons are nested inside a DIV).
    const nativeActions=nativeAdd.parentElement;
    if(nativeActions && nativeActions !== root){nativeActions.style.display='none';root.appendChild(box)}else{nativeAdd.style.display='none';root.appendChild(box)}
