(function(){
  ...
  function setup(){
    ...
    if(!header.querySelector('.referral-header-button')){
      ...
      b.className='referral-header-button';
      b.type='button';
      b.setAttribute('aria-label','Refer and Earn');
      b.title='Refer & Earn';
    ...
    // CSS elsewhere previously gave the brand/cart their own flex order.
    // Force the requested order regardless of when the React/legacy buttons mount:
    // Profile -> Refer & Earn -> Save -> Cart, with search on the next row.
    if(brand) brand.style.setProperty('order','1','important');
    if(referral) referral.style.setProperty('order','2','important');
    if(wish) wish.style.setProperty('order','3','important');
    if(cart){
      cart.style.setProperty('order','4','important');
      cart.style.setProperty('margin-left','auto','important');
    }
    if(search) search.style.setProperty('order','5','important');
  }

  function openReferral(){
    var old=document.getElementById('referral-panel');
    if(old){old.classList.add('open');return}
    var user={};
    try{user=JSON.parse(localStorage.getItem('apna-cart-user')||'{}')}catch(e){}
    var contact=String(user.contact||'').replace(/\D/g,'').slice(-6)||'APNA';
    var code='APNA'+contact;
    var p=document.createElement('div');
    p.id='referral-panel';
    p.className='referral-panel open';
  ...
  function start(){
    setup();
    new MutationObserver(function(){setup()}).observe(document.body,{childList:true,subtree:true});
  }
