(() => {
  ...
  async function sync() {
    ...
    } catch (_) { /* Keep the existing frontend catalog if backend is unavailable. */ }
  }
  const boot = () => {
    const observer = new MutationObserver(() => {
      clearTimeout(timer); timer = setTimeout(sync, 120)
    })
